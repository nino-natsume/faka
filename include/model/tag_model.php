...
class Tag_Model {
    ...
    function getNamesFromIds($tagIds = NULL) {
        ...
        $tag_string = implode(',', $cleanTagIds);
        $sql = "SELECT `tid`, `tagname` FROM `$this->table` WHERE `tid` IN (" . $tag_string . ")";
        $query = $this->db->query($sql);
    ...
    function getNamesFromIdStr($tagIdStr = '') {
        ...
        $tagIds = [];
        foreach (explode(',', (string)$tagIdStr) as $tagId) {
        ...
        $sql = "SELECT `tid`, `tagname` FROM `$this->table` WHERE `tid` IN (" . implode(',', $tagIds) . ")";
        $query = $this->db->query($sql);
