...
class Comment_Model {
    ...
    function getComments($blogId = null, $hide = null, $page = null) {
        $blogId = (int)$blogId;
        $page = max(1, (int)$page);
        $hide = in_array($hide, ['y', 'n'], true) ? $hide : '';
        $andQuery = '1=1';
        $andQuery .= $blogId ? " and a.gid=$blogId" : '';
        $andQuery .= $hide ? " and a.hide='$hide'" : '';
        $condition = '';

        $orderBy = Option::get('comment_order') == 'newer' ? 'ORDER BY a.top ASC, a.date ASC' : 'ORDER BY a.top DESC, a.date ASC';
        $sql = "SELECT * FROM " . DB_PREFIX . "comment as a where $andQuery $orderBy $condition";
        ...
        if (Option::get('comment_paging') == 'y') {
            $commentPnum = (int)Option::get('comment_pnum');
            $commentPnum = $commentPnum > 0 ? $commentPnum : 10;
            $totalPages = count($commentStacks) > 0 ? ceil(count($commentStacks) / $commentPnum) : 1;
            if ($page > $totalPages) {
                $page = $totalPages;
            }
            $pageurl = method_exists('Url', 'entry') ? Url::entry($blogId) : Url::log($blogId);
            if (Option::get('isurlrewrite') == 0 && strpos($pageurl, '=') !== false) {
                $pageurl .= '&comment-page=';
            } else {
                $pageurl .= '/comment-page-';
            }
            $commentPageUrl = pagination(count($commentStacks), $commentPnum, $page, $pageurl, '#comments');
            $commentStacks = array_slice($commentStacks, ($page - 1) * $commentPnum, $commentPnum);
        }
     ...
     */
    function getCommentsForAdmin($blogId = null, $uid = null, $hide = null, $page = null, $keyword = '', $perPageNum = 0) {
        $blogId = (int)$blogId;
        $uid = (int)$uid;
        $page = max(1, (int)$page);
        $hide = in_array($hide, ['y', 'n'], true) ? $hide : '';
        ...
        $andQuery .= $hide ? " and a.hide='$hide'" : '';
        if ($keyword !== '') {
            $keywordSql = addslashes($keyword);
            $andQuery .= " and (a.comment like '%$keywordSql%' or a.poster like '%$keywordSql%' or a.mail like '%$keywordSql%' or a.ip like '%$keywordSql%' or b.title like '%$keywordSql%')";
        }
        $condition = '';
        if ($page) {
            $per_page_num = (int)$perPageNum > 0 ? (int)$perPageNum : (int)Option::get('admin_article_perpage_num');
            $per_page_num = $per_page_num > 0 ? $per_page_num : 20;
            if ($page > PHP_INT_MAX) {
                $page = PHP_INT_MAX;
            }
    ...
    function getCommentNum($blogId = null, $uid = null, $hide = null, $keyword = '') {
        $blogId = (int)$blogId;
        $uid = (int)$uid;
        $hide = in_array($hide, ['y', 'n'], true) ? $hide : '';
        $keyword = trim((string)$keyword);
        ...
        if (User::haveEditPermission() && $keyword === '') {
            ...
            if (User::haveEditPermission()) {
                $sql = "SELECT count(*) FROM " . DB_PREFIX . "comment as a, " . DB_PREFIX . "blog as b where $andQuery and a.gid=b.gid";
            }
        }
    ...
    function addComment($uid, $name, $content, $mail, $url, $blogId, $pid) {
        ...
        if ($pid > 0) {
            ...
            if (!$comment || (int)$comment['gid'] !== $blogId) {
                $pid = 0;
            } else {
