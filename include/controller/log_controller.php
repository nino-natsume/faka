...
class Log_Controller
{
    ...
    function display($params)
    {
        ...
        if (isset($params[2]) && $params[2] === 'page') {
            $page = abs((int)$params[3]);
        } elseif (isset($params[1]) && $params[1] === 'page') {
            $page = abs((int)$params[2]);
        }
        $page = max(1, $page);

        $pageurl = '';
        $sqlSegment = 'ORDER BY top DESC ,date DESC';
        $lognum = $Log_Model->getLogNum('n');
        $pageurl .= Url::blogPage();
...
//echo 2;die;
    ...
    function displayContent($params)
    {
        ...
        if ($type === 'blog') {
            ...
            $template = !empty($template) && file_exists(BLOG_TEMPLATE_PATH . $template . '.php') ? $template : 'echo_log';
            include View::getBlogView($template);
        } elseif ($type === 'page') {
            $template = !empty($template) && file_exists(BLOG_TEMPLATE_PATH . $template . '.php') ? $template : 'page';
            include View::getBlogView($template);
        }
    }
    ...
    function addComment()
    {
        ...
        if (!ISLOGIN && Option::get('login_comment') === 'y') {
            $commentReturnUrl = '';
            if (!empty($_SERVER['HTTP_REFERER']) && function_exists('dcGetSafeRedirectUrl')) {
                $commentReturnUrl = dcGetSafeRedirectUrl($_SERVER['HTTP_REFERER'], '');
            }
            if ($commentReturnUrl === '') {
                $commentReturnUrl = isset($log['type']) && $log['type'] === 'page' ? Url::art($blogId) : Url::log($blogId);
            }
            $commentLoginUrl = function_exists('dcGetUserLoginUrl') ? dcGetUserLoginUrl($commentReturnUrl) : DC_URL . 'user/account.php?action=signin';
            $resp === 'json' ? Output::error('请先登录用户中心后再评论') : emDirect($commentLoginUrl);
            ...
            $err = '内容内容太长了';
        } elseif ($pid > 0 && (($parentComment = $Comment_Model->getOneComment($pid)) === false || (int)$parentComment['gid'] !== (int)$blogId)) {
            $err = '回复的评论不存在';
        ...
        if ($resp === 'json') {
            ...
            $redirectUrl = isset($log['type']) && $log['type'] === 'page' ? Url::art($blogId) : Url::log($blogId);
            emDirect($redirectUrl . '#' . $cid);
        }
    }
