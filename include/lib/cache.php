...
class Cache {
    ...
    public function updateArticleCache() {
        $this->updateCache(['sta', 'tags', 'blog_sort', 'newlog', 'record', 'logsort', 'logalias']);
    }
    ...
    public function readCache($cacheName) {
        ...
        if($cacheName == 'blog_navi'){
            $blogNaviModel = new Blog_Navi_Model();

            $res = $blogNaviModel->getNavis();
            $list = [];
            foreach($res as &$val){
                if($val['hide'] == 'n') {
                    $list[] = $val;
        ...
        $cacheFile = DC_ROOT . '/content/cache/' . $cacheName . '.php';
        ...
        if ($cacheData = @file_get_contents($cacheFile)) {
            clearstatcache();
            $data = @unserialize(str_replace("<?php exit;//", '', $cacheData));
            if ($data === false) {
                // 缓存数据损坏，重新生成
                if (method_exists($this, 'mc_' . $cacheName)) {
                    $this->{'mc_' . $cacheName}();
    ...
    private function mc_user() {
        ...
        while ($row = $this->db->fetch_array($query)) {
            ...
            $user_cache[$row['uid']] = [
                ...
                'role'      => $row['role'],
                'admin_group_id' => isset($row['admin_group_id']) ? (int)$row['admin_group_id'] : 0,
            ];
        }
