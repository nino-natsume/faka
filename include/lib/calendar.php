...
class Calendar {
    ...
    static function url() {
        $calendarUrl = isset($GLOBALS['record']) ? self::blogUrl('?action=cal&record=' . (int)$GLOBALS['record']) : self::blogUrl('?action=cal');
        return $calendarUrl;
    }

    private static function blogUrl($path = '') {
        return function_exists('dcGetBlogBaseUrl') ? dcGetBlogBaseUrl($path) : rtrim(DYNAMIC_BLOGURL, '/') . '/' . ltrim($path, '/');
    }
    ...
    static function generate() {
        ...
        $url = self::blogUrl('?action=cal&record=' . ($n_year - 1) . $n_month); //上一年份
        $url2 = self::blogUrl('?action=cal&record=' . ($n_year + 1) . $n_month);//下一年份
        $url3 = self::blogUrl('?action=cal&record=' . $year_down . $m);         //上一月份
        $url4 = self::blogUrl('?action=cal&record=' . $year_up . $mj);          //下一月份
