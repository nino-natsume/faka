...
class Storage {
    ...
    public function setValue($name, $value = NULL, $type = NULL) {
        ...
        if ($this->isStationTemplateContext()) {
            $this->setStationValue($name, $db_value, $type);
            return;
        }
    ...
    public function getValue($name) {
        ...
        if ($this->isStationTemplateContext()) {
            $stationValue = $this->getStationValue($name);
            if ($stationValue !== false) {
                return $stationValue;
            }
        }
    ...
    private function isStationTemplateContext()
    {
        if (!$this->isTemplatePlugin()) {
            return false;
        }
        return $this->getStationContextId() > 0;
    }

    private function isTemplatePlugin()
    {
        if (!defined('TPLS_PATH')) {
            return false;
        }
        $safe = preg_replace('/^([\w-]+)$/i', '$1', $this->plugin_name);
        if ($safe !== $this->plugin_name || $safe === '') {
            return false;
        }
        $path = rtrim(TPLS_PATH, '/\\') . '/' . $safe . '/';
        if (!is_dir($path)) {
            return false;
        }
        if (function_exists('checkTemplateBootstrap')) {
            return checkTemplateBootstrap(TPLS_PATH, $safe);
        }
        return file_exists($path . $safe . '.php');
    }
    ...
    private function getStationContextId()
    {
        ...
        if (substr($script, -18) === '/user/template.php' && !empty($userData['station']['id'])) {
            return (int)$userData['station']['id'];
        }
        return 0;
    }

    private function setStationValue($name, $db_value, $type)
    {
    ...
    private function getStationValue($name)
    {
        $stationId = $this->getStationContextId();
        if ($stationId <= 0) {
            return false;
        }
        $plugin = $this->db_conn->escape_string($this->plugin_name);
        $option = $this->db_conn->escape_string($name);
        $sql = "SELECT `option_value` FROM " . DB_PREFIX . "station_storage WHERE `station_id` = " . (int)$stationId . " AND `type` = 'tpl' AND `plugin_name` = '" . $plugin . "' AND `option_name` = '" . $option . "'";
        $result = $this->db_conn->once_fetch_array($sql);
        if (empty($result)) {
            return false;
        }
        ...
        if (is_array($payload) && array_key_exists('__storage_type', $payload)) {
            $type = $payload['__storage_type'];
            $value = $payload['value'];
            switch ($type) {
                case 'string':
                    return (string)$value;
