## 当前进度（60%）
**已完成：** 网站前端、登录&注册界面、支付页面及其接口等

**未完成：** 部署变量更新、管理后端页面样式、杂乱代码清理等
...
基于 Acg-faka 改造的 Cloudflare 版本，使用 **Workers + D1** ，尽可能还原，实现商品浏览、下单、支付、自动发货、订单查询与管理后台，前台/后台/订单闭环开箱即用。
...
| Secret | 必填 | 说明 |
|---|---|---|
| `CLOUDFLARE_API_TOKEN` | 是 | Cloudflare API Token（My Profile → API Tokens 创建）。<br>所需权限：<br>Account › Workers Scripts › Edit<br>Account › D1 › Edit<br>Account › Account Settings › Read<br>Zone › Zone › Read<br>Zone › Workers Routes › Edit |
...
3. **初始化数据库**：进入刚创建的 `faka` → `Console`（或 `Import`），
   依次粘贴执行 [schema.sql](schema.sql) 与 [seed.sql](seed.sql) 的内容。
...
6. **配置变量(Secret)**：`Settings` → `Environment variables` → `Add`，
   添加三项：
   - `SECRET`：一段 32+ 位随机串
...
> 若修改 `worker.js` / `admin.js` / `lib.js`：待代码 push 到 `main`
> 后，通过运行`Build Pages Bundle` 工作流重新生成新 `public/_worker.js` 并提交。
