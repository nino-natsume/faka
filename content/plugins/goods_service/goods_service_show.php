...
<?php if(User::isAdmin() && $action == 'del'): ?>
    ...
    $res = $db->fetch_all("select id, sku from {$db_prefix}goods_service where id in ({$ids})");
...
//    output::error('test');
    try {
        $db->beginTransaction();
        if($sale == 'n'){
            $db->query("DELETE FROM {$db_prefix}goods_service WHERE id IN ({$ids})");
            foreach($res as $val){
                foreach($skus as $v){
                    if($v['sku'] == $val['sku']){
            ...
            $db->query("DELETE FROM {$db_prefix}goods_service_sale WHERE id IN ({$ids})");
        }
        $db->commit();
    } catch (Exception $e) {
        $db->rollback();
...
$exists_template = $db->once_fetch_array("select id from {$db_prefix}goods_service where goods_id = {$goods_id} and sku = '{$sku}' order by id asc");
...
try {
    ...
    $db->query("delete from {$db_prefix}goods_service where goods_id = {$goods_id} and sku = '{$sku}'");
    $db->query("INSERT INTO `{$db_prefix}goods_service` (`goods_id`, `sku`, `content`, `create_time`) VALUES ({$goods_id}, '{$sku}','{$content}', {$timestamp})");
...
$template_list = $db->fetch_all("select id, sku from {$db_prefix}goods_service where goods_id = {$goods_id} order by id asc");
$template_sku_map = [];
...
<?php include View::getAdmView('open_head'); ?>
    ...
    <form class="layui-form " action="/?plugin=goods_service&action=add_ajax" id="form">
        <div style="padding: 25px;" id="open-box">
            ...
            <div class="layui-form-item">
                <label class="layui-form-label">购买后默认显示内容</label>
                <div class="layui-input-block">
                    <textarea rows="8" class="layui-textarea" name="content" <?= $all_template_locked ? 'disabled' : '' ?>>已收到您的订单，客服将会尽快为您处理！</textarea>
                </div>
            </div>
...
//output::error('测试');
    try {
        ...
        if($type == 'ws'){
            ...
            $db->query("UPDATE `{$db_prefix}goods_service_sale` SET `content` = '{$content}' WHERE `id` = {$stock_id}");
        }
...
<?php if(User::isAdmin() && $action == 'edit'): ?>
    ...
    if($type == 'ws'){
        $stock = $db->once_fetch_array("select * from {$db_prefix}goods_service where id = {$stock_id}");
    }else{
        $stock = $db->once_fetch_array("select * from {$db_prefix}goods_service_sale where id = {$stock_id}");
    }
...
//    d($skus);die;
    ...
    <form class="layui-form " action="/?plugin=goods_service&action=edit_ajax" id="form">
        <div style="padding: 25px;" id="open-box">
...
if($sale == 'y'){
    $list = $db->fetch_all("select * from {$db_prefix}goods_service_sale {$where} and is_default = 'n' order by id desc limit {$limit_start}, {$page_count}");
    $count = $db->once_fetch_array("select count(id) kami_count from {$db_prefix}goods_service_sale {$where} and is_default = 'n'")['kami_count'];
}else{
    $list = $db->fetch_all("select * from {$db_prefix}goods_service {$where} order by id desc limit {$limit_start}, {$page_count}");
    $count = $db->once_fetch_array("select count(id) kami_count from {$db_prefix}goods_service {$where}")['kami_count'];
}
...
<script>
    layui.use(['table'], function(){
        ...
        window.ws_table = table.render({
            elem: '#stock_index_ws',
            autoSort: false,
            url: '/?plugin=goods_service&action=index&sale=n&goods_id=<?= $goods_id ?>', // 此处为静态模拟数据，实际使用时需换成真实接口
            toolbar: '#toolbar',
            ...
            cols: [[
                ...
                {field:'content', title:'默认显示', align: 'center', minWidth: 200 },
        ...
        table.on('toolbar(stock_index_ws)', function(obj){
            ...
            if(obj.event == 'del'){
                ...
                layer.confirm('确定要删除的数据？', {
                    ...
                    $.ajax({
                        url: '/?plugin=goods_service&action=del',
                        type: 'POST',
                        dataType: 'json',
            ...
            if(obj.event == 'add'){
                ...
                layer.open({
                    ...
                    content: '/?plugin=goods_service&action=add&goods_id=<?= $goods_id ?>',
                    fixed: false, // 不固定
                    maxmin: true,
                    shadeClose: true,
            ...
            if(obj.event == 'export'){
                ...
                layer.open({
                    ...
                    // skin: 'layui-layer-win10',
                    skin: 'layui-layer-molv',
                    content: '/?plugin=goods_service&action=export&sale=n&goods_id=<?= $goods_id ?>',
                    fixed: false, // 不固定
                    maxmin: true,
                    shadeClose: true,
                    success: function(layero, index, that){
        ...
        table.on('tool(stock_index_ws)', function(obj){ // 双击 toolDouble
            ...
            if(obj.event === 'edit'){
                ...
                layer.open({
                    ...
                    // skin: 'layui-layer-win10',
                    skin: 'layui-layer-molv',
                    content: '/?plugin=goods_service&goods_id=<?= $goods_id ?>&action=edit&stock_id=' + data.id,
                    fixed: false, // 不固定
                    maxmin: true,
                    shadeClose: true,
                    success: function(layero, index, that){
    ...
    <script>
        layui.use(['table'], function(){
            ...
            window.ws_table = table.render({
                elem: '#stock_index_ws',
                autoSort: false,
                url: '/?plugin=goods_service&action=index&sale=y&goods_id=<?= $goods_id ?>', // 此处为静态模拟数据，实际使用时需换成真实接口
                toolbar: '#toolbar',
                limits: [10,20,30,50,100],
                page: true,
                lineStyle: 'height: 30px;',
            ...
            table.on('toolbar(stock_index_ws)', function(obj){
                ...
                if(obj.event == 'del'){
                    ...
                    layer.confirm('确定要删除的数据？', {
                        ...
                        layer.close(index); // 关闭对话框
                        $.ajax({
                            url: '/?plugin=goods_service&action=del&sale=y',
                            type: 'POST',
                            dataType: 'json',
                            data: { ids: ids, goods_id: "<?= $goods_id ?>", token: '<?= LoginAuth::genToken() ?>' },
                            success: function(e) {
            ...
            table.on('tool(stock_index_ws)', function(obj){ // 双击 toolDouble
                ...
                if(obj.event === 'edit'){
                    ...
                    layer.open({
                        ...
                        // skin: 'layui-layer-win10',
                        skin: 'layui-layer-molv',
                        content: '/?plugin=goods_service&type=ys&action=edit&stock_id=' + data.id,
                        fixed: false, // 不固定
                        maxmin: true,
                        shadeClose: true,
                        success: function(layero, index, that){
...
<?php endif; ?>
<?php if(User::isAdmin() && $action == 'deliver_ajax'): ?>
    <?php
    $order_id = Input::postIntVar('order_id');
    $order_list_id = Input::postIntVar('order_list_id');
    $order_list = $db->once_fetch_array("select * from {$db_prefix}order_list where id = {$order_list_id}");
    $content = Input::postStrVar('content');
    if(empty($content)){
        output::error('发货内容不能为空');
    }
    try {
        $db->beginTransaction();
    ...
    (goods_id, order_list_id, sku, content, num, is_default, create_time) 
    value 
    ({$order_list['goods_id']}, {$order_list_id}, '{$order_list['sku']}', '{$content}', {$order_list['quantity']}, 'n', {$timestamp})
    ");
