...
<style>
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .auth-captcha img{height:44px;border-radius:10px;cursor:pointer;border:1px solid #e5e6eb;transition:transform .2s;}
    .auth-captcha img:hover{transform:scale(1.03);}
    .auth-btn{
...
<div class="auth-card">
    <div class="auth-header">
        <div class="auth-avatar"><i class="ri-mail-send-line"></i></div>
        <div class="auth-title">找回密码</div>
        <div class="auth-tip">请输入注册时使用的邮箱地址</div>
    </div>

    <div class="auth-body">
        <?php if (isset($_GET['error_mail'])): ?>
            <div class="auth-alert error"><i class="ri-error-warning-line"></i> 错误的注册邮箱</div>
        <?php endif ?>
        <?php if (isset($_GET['error_sendmail'])): ?>
            <div class="auth-alert error"><i class="ri-error-warning-line"></i> 邮件验证码发送失败，请检查邮件通知设置</div>
        <?php endif ?>
        <?php if (isset($_GET['err_ckcode'])): ?>
            <div class="auth-alert error"><i class="ri-error-warning-line"></i> 图形验证码错误</div>
        <?php endif ?>
        ...
        <form method="post" action="./account.php?action=doreset">
            ...
            <button type="submit" class="auth-btn">发送验证码</button>
