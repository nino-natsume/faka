...
<head>
    ...
    <title>重置密码 - <?= htmlspecialchars($blogname) ?></title>
...
.m-step{
    ...
    margin-bottom:20px;font-size:12px;color:#c9cdd4;
}
.m-step .step{display:flex;align-items:center;gap:4px;}
.m-step .step.done{color:#34c759;}
.m-step .step.active{color:#2196F3;font-weight:600;}
.m-step .step-line{width:24px;height:1px;background:#e5e6eb;}
...
<div class="m-auth-page">
    ...
    <div class="m-auth-header">
        <div class="m-auth-logo"><i class="ri-key-2-line"></i></div>
        <div class="m-auth-site">重置密码</div>
        <div class="m-auth-tip">请查收邮箱中的验证码并设置新密码</div>
    </div>
    ...
    <div class="m-auth-body">
        ...
        <?php if (isset($_GET['succ_mail'])): ?>
            <div class="m-alert success"><i class="ri-checkbox-circle-line"></i> 邮件验证码已发送，请查收邮箱</div>
        <?php endif ?>
        <?php if (isset($_GET['err_mail_code'])): ?>
            <div class="m-alert error"><i class="ri-error-warning-line"></i> 邮件验证码错误</div>
        <?php endif ?>
        <?php if (isset($_GET['error_pwd_len'])): ?>
            <div class="m-alert error"><i class="ri-error-warning-line"></i> 密码长度不合规</div>
        <?php endif ?>
        <?php if (isset($_GET['error_pwd2'])): ?>
            <div class="m-alert error"><i class="ri-error-warning-line"></i> 两次输入的密码不一致</div>
        <?php endif ?>

        <form method="post" action="./account.php?action=doreset2">
            <div class="m-field">
                <i class="f-icon ri-mail-open-line"></i>
            ...
            <button type="submit" class="m-auth-btn">重置密码</button>
        </form>
    </div>
