...
<style>
    ...
    *{margin:0;box-sizing:border-box;}
    ...
    .mac-avatar{
        ...
        box-shadow:0 8px 18px rgba(245,158,11,.3),inset 0 0 0 2px rgba(255,255,255,.7);
    }
    ...
    .mac-captcha{display:flex;gap:8px;margin-bottom:10px;align-items:center;}
    .mac-captcha .mac-field{flex:1;margin:0;}
    .mac-captcha img{height:42px;border-radius:10px;border:1px solid rgba(0,0,0,.08);cursor:pointer;transition:all .2s ease;background:#fff;}
...
<div class="mac-window" role="dialog" aria-label="找回密码" style="background-image:url('<?= htmlspecialchars($__signin_bg, ENT_QUOTES) ?>');">
    <div class="mac-titlebar">
        ...
        <div class="mac-title">找回密码</div>
        <div style="width:52px;"></div>
    </div>

    <div class="mac-body">
        <div class="mac-avatar"><i class="ri-mail-send-line"></i></div>
        <div class="mac-page-title">验证邮箱</div>
        <div class="mac-page-tip">输入注册邮箱，我们将发送验证码</div>

        <?php if (isset($_GET['error_mail'])): ?>
            <div class="mac-alert error"><i class="ri-error-warning-line"></i> 错误的注册邮箱</div>
        <?php endif ?>
        <?php if (isset($_GET['error_sendmail'])): ?>
            <div class="mac-alert error"><i class="ri-error-warning-line"></i> 邮件验证码发送失败，请检查邮件通知设置</div>
        <?php endif ?>
        <?php if (isset($_GET['err_ckcode'])): ?>
            <div class="mac-alert error"><i class="ri-error-warning-line"></i> 图形验证码错误</div>
        <?php endif ?>

        <form method="post" class="layui-form mac-form" action="./account.php?action=doreset">
            <div class="mac-field">
                <i class="mac-icon ri-mail-line"></i>
                <input type="email" class="mac-input" id="mail" name="mail" placeholder="注册邮箱" required autofocus>
            </div>
            ...
            <div class="mac-submit-wrap">
                <button type="submit" class="mac-go" title="发送验证码">
...
<div class="mac-dock">
    ...
    <?php if ($login_code): ?>
    <a href="javascript:document.getElementById('checkcode')&&document.getElementById('checkcode').click();" title="换一张验证码">
        <i class="ri-shield-keyhole-line"></i>
        <span class="tip">换验证码</span>
    </a>
    <?php endif ?>
    <span class="sep"></span>
...
<script>
    $(function () {
        ...
        $('#checkcode').on('click', function () {
            $(this).attr("src", "../include/lib/checkcode.php?" + new Date().getTime());
        });
