...
<div class="layui-tabs order-tabs-wrapper" style="display:flex;align-items:center;justify-content:space-between;" lay-options="{trigger: false}">
    <ul class="layui-tabs-header">
        ...
        <li class="layui-this"><a href="./store.php?action=plu">扩展插件</a></li>
    ...
    <div style="display:flex;align-items:center;gap:6px;flex-shrink:0;">
        <div class="layui-input-inline layui-input-wrap" style="width:180px;margin:0;">
            <input type="text" class="layui-input" id="store-search" placeholder="搜索插件..." lay-affix="clear">
        </div>
...
<div class="store-category-bar">
    <div class="category-tabs">
        <?php foreach ($plugin_type_arr as $cat): ?>
        <span class="tab-item <?= $plugin_type == $cat['id'] ? 'active' : '' ?>" data-id="<?= $cat['id'] ?>"><?= htmlspecialchars($cat['title']) ?></span>
        <?php endforeach; ?>
    </div>
</div>
...
<div class="layui-table-view" style="border-radius:8px;overflow:hidden;">
    <div class="layui-card-header" style="display:flex;align-items:center;justify-content:center;position:relative;height:auto;padding:12px 15px;">
        ...
        <span style="color:#667797;font-size:14px;font-weight:500;">扩展插件</span>
...
<script>
var currentCategory = <?= (int)$plugin_type ?>;
var storeData = [];
...
function renderStoreCards(pageData) {
    ...
    if (dataToRender.length === 0 && allStoreData.length === 0) {
        $('#cards-container').removeClass('active').css({'display':'flex','justify-content':'center','align-items':'center','min-height':'300px'}).html('<div style="text-align:center;padding:50px 0;color:#999;"><i class="ri-apps-line" style="font-size:48px;display:block;margin-bottom:10px;"></i>暂无插件</div>');
    ...
    dataToRender.forEach(function(d) {
        var coverImg = d.cover ? '<img src="' + d.cover + '" onerror="this.parentNode.innerHTML=\'<i class=\\\'ri-apps-line default-icon\\\'></i>\'">' : '<i class="ri-apps-line default-icon"></i>';
...
function loadStoreData(callback, page, limit) {
    ...
    $.ajax({
        url: '?action=plu_ajax&plugin_type=' + currentCategory + '&keyword=' + encodeURIComponent(currentKeyword) + '&page=' + page + '&limit=' + limit, type: 'GET', dataType: 'json',
...
function renderCardPagination() {
    if (allStoreData.length === 0) {
        // 显示空数据提示
        $('#cards-container').removeClass('active').css({'display':'flex','justify-content':'center','align-items':'center','min-height':'300px'}).html('<div style="text-align:center;padding:50px 0;color:#999;"><i class="ri-apps-line" style="font-size:48px;display:block;margin-bottom:10px;"></i>该分类下暂无插件</div>');
        $('#cards-pagination').hide();
        return;
    }
...
layui.use(['table'], function(){
    ...
    $('.category-tabs .tab-item').on('click', function(){
        ...
        table.reload('index', { page: { curr: 1 }, where: { plugin_type: id } });
        if (currentView === 'card') {
            cardCurrentPage = 1;
    ...
    table.render({
        elem: '#index', url: '?action=plu_ajax&plugin_type=' + currentCategory, page: true, limits: [10, 20, 50], lineStyle: 'height: 65px;',
        text: { none: '该分类下暂无插件' },
        cols: [[
            {field: 'cover', title: '图标', width: 75, templet: '#cover', align: 'center'},
            {field: 'name', title: '插件名称', minWidth: 350, templet: '#name'},
    ...
    $('#store-reset-btn').on('click', function() {
        ...
        currentCategory = 0;
        currentKeyword = '';
        table.reload('index', { page: { curr: 1 }, url: '?action=plu_ajax&plugin_type=0', where: { keyword: '' } });
        if (currentView === 'card') {
            cardCurrentPage = 1;
            loadStoreData(function(){ renderCardPagination(); }, 1, cardPageSize);
        }
    ...
    function filterStorePlugins(keyword) {
        ...
        if (currentView === 'card') {
            ...
            table.reload('index', { page: { curr: 1 }, where: { plugin_type: currentCategory, keyword: keyword } });
        }
    }
    ...
    if (initKeyword) {
        $('#store-search').val(initKeyword);
        setTimeout(function() {
            filterStorePlugins(initKeyword);
        }, 500);
    }
});
 ...
 * @param {function} onSuccess - 安装成功回调
 * @param {function} onFail - 安装失败回调
 */
    ...
    $.post('./store.php?action=install', { type: type, plugin_id: pluginId }, function(res) {
        ...
        var pollTimer = setInterval(function() {
            ...
            if (pollCount > 90) {
                ...
                layer.close(loadIdx);
                if (onFail) onFail('安装超时，请重试');
                else layer.msg('安装超时，请重试', {icon: 2});
                return;
            }
            $.ajax({
                url: './store.php?action=install_progress',
                type: 'GET',
                dataType: 'json',
                timeout: 5000,
                success: function(pRes) {
                    ...
                    if (task.status === 'completed') {
                        ...
                        clearInterval(pollTimer);
                        layer.close(loadIdx);
                        if (onFail) onFail(task.error || '安装失败');
                        else layer.msg(task.error || '安装失败', {icon: 2});
...
function handleAction(event, data, table) {
    ...
    if(event === 'install'){
        asyncInstallApp(data.id, data.type || 'plugin', data.name, function(){
            layer.open({
                type: 1, title: '安装成功', area: '400px',
                content: '<div style="padding:20px;text-align:center;"><i class="ri-checkbox-circle-fill" style="font-size:48px;color:#16a34a;"></i><div style="margin:15px 0;font-size:16px;">插件「' + data.name + '」已安装</div></div>',
                btn: ['去启用', '稍后再说'],
                yes: function(i) { layer.close(i); location.href = './plugin.php?search=' + encodeURIComponent(data.name); },
    ...
    if(event === 'buy'){
            ...
            $.post('./store.php?action=buy', { app_id: data.id, buy_type: 'permanent', months: 1 }, function(res){
                ...
                if(res.code == 0) {
                    layer.open({
                        ...
                        content: '<div style="padding:20px;text-align:center;"><i class="ri-checkbox-circle-fill" style="font-size:48px;color:#16a34a;"></i><div style="margin:15px 0;font-size:16px;">插件「' + data.name + '」已添加到您的应用列表</div></div>',
                        btn: ['立即安装', '稍后再说'],
                        yes: function(idx) {
                            layer.close(idx);
                            asyncInstallApp(data.id, 'plugin', data.name, function(){
                                layer.open({
                                    type: 1, title: '安装成功', area: '400px',
                                    content: '<div style="padding:20px;text-align:center;"><i class="ri-checkbox-circle-fill" style="font-size:48px;color:#16a34a;"></i><div style="margin:15px 0;font-size:16px;">插件「' + data.name + '」已安装</div></div>',
                                    btn: ['去启用', '稍后再说'],
                ...
                $.post('./store.php?action=buy', { app_id: data.id, buy_type: 'permanent', months: 1 }, function(res){
                    ...
                    if(res.code == 0) {
                        layer.open({
                            ...
                            title: '购买成功',
                            area: '400px',
                            content: '<div style="padding:20px;text-align:center;"><i class="ri-checkbox-circle-fill" style="font-size:48px;color:#16a34a;"></i><div style="margin:15px 0;font-size:16px;">插件「' + data.name + '」已买断</div></div>',
                            btn: ['立即安装', '稍后再说'],
                            yes: function(idx) {
                                layer.close(idx);
                                asyncInstallApp(data.id, data.type || 'plugin', data.name, function(){
        ...
        layer.open({ 
            ...
            $.post('./store.php?action=buy', { app_id: data.id, buy_type: buyType, months: months }, function(res){
                ...
                var successText = buyType === 'monthly' ? '插件「' + data.name + '」已订阅' + months + '个月' : '插件「' + data.name + '」已购买';
                if(res.code == 0) {
                    layer.open({
                        type: 1,
                        title: successTitle,
...
function showAppDetail(app) {
    ...
    var html = '<div class="app-detail-modal" onclick="closeAppDetail()"><div class="app-detail-box" onclick="event.stopPropagation()"><div class="app-detail-header">' +
        ...
        '<div class="app-detail-meta"><span><i class="ri-plug-line"></i> 插件</span><span><i class="ri-price-tag-3-line"></i> v' + app.version + '</span><span><i class="ri-user-line"></i> ' + escapeHtml(app.author || '未知') + '</span></div>' +
