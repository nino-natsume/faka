...
<script type="text/html" id="toolbar">
    <div style="display:flex;align-items:center;justify-content:center;position:relative;padding:10px 0px 15px;border-bottom:1px solid #f0f0f0;">
        ...
        <span style="color:#667797;font-size:14px;font-weight:500;">页面管理</span>
    </div>
    <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
        ...
        <form class="layui-form" style="display: flex; align-items: center; gap: 6px; margin: 0; flex-wrap: wrap; justify-content: flex-end;">
            ...
            <div class="layui-input-inline layui-input-wrap" style="width: 160px; margin: 0;">
                <input type="text" value="" name="keyword" placeholder="搜索页面标题..." lay-affix="clear" class="layui-input">
            </div>
...
<script type="text/html" id="pageTitleTpl">
    <div class="layui-clear-space">
        <span>{{ d.title }}</span>
        {{# if(d.home_page === 'y') { }}
            <span class="layui-badge layui-bg-blue">首页</span>
        {{# } }}
    </div>
</script>
...
<script type="text/html" id="operate">
    <div class="layui-clear-space">
        <a class="layui-btn layui-btn-primary" lay-event="view">预览</a>
    ...
    layui.use(['table', 'form'], function(){
        ...
        window.table = table.render({
            ...
            cols: [[
                ...
                {field:'title', title: '标题', minWidth: 180, templet: '#pageTitleTpl'},
                {field:'alias', title:'别名', width: 120, templet: '#aliasTpl'},
                {field:'status_text', title:'状态', width: 90, templet: '#statusTpl', align: 'center'},
                {field:'views', title:'浏览数', width: 90, align: 'center', sort: true},
                {field:'comnum', title:'评论数', width: 90, align: 'center', sort: true},
                {field:'allow_remark_text', title:'评论', width: 80, align: 'center'},
                {field:'template', title:'模板', width: 100},
                {field:'date', title:'添加时间', sort: true, width: 150},
                {fixed: 'right', title:'操作', templet: '#operate', width: 250, align: 'center'}
            ]],
        ...
        table.on('toolbar(index)', function(obj){
            ...
            if(obj.event == 'add'){
                ...
                let area = isMobile ? ['98%', '90%']  : ['1000px', '85%'];
                layer.open({
                    id: 'edit',
                    title: '添加',
                    type: 2,
...
<script>
    $(function () {
        $("#menu-blog").attr('class', 'admin-menu-item has-submenu open');
        $("#menu-blog .admin-arrow").addClass('active');
    });
</script>
