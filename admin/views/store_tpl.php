...
.category-tabs { display: inline-flex; flex-wrap: wrap; gap: 2px; padding: 3px; background: #f0f1f4; border-radius: 8px; }
.category-tabs .tab-item { padding: 5px 14px; border-radius: 6px; cursor: pointer; font-size: 13px; color: #666; background: transparent; border: none; transition: all 0.2s; white-space: nowrap; }
.category-tabs .tab-item:hover { color: #333; background: rgba(255,255,255,.5); }
.category-tabs .tab-item.active { color: #333; font-weight: 500; background: #fff; box-shadow: 0 1px 3px rgba(0,0,0,.08), 0 0 0 0.5px rgba(0,0,0,.04); }
html[data-theme="dark"] .category-tabs { background: #2a2a2a; }
html[data-theme="dark"] .category-tabs .tab-item { color: #b0b0b0; }
html[data-theme="dark"] .category-tabs .tab-item:hover { color: #e2e8f0; background: rgba(255,255,255,.06); }
html[data-theme="dark"] .category-tabs .tab-item.active { color: #e0e0e0; background: #3a3a3a; box-shadow: 0 1px 3px rgba(0,0,0,.3); }
...
@media (max-width: 1400px) {
    .store-cards.active { grid-template-columns: repeat(4, 1fr); }
}
@media (max-width: 1100px) {
    .store-cards.active { grid-template-columns: repeat(3, 1fr); }
}
@media (max-width: 768px) {
    .store-cards.active { grid-template-columns: repeat(2, 1fr); gap: 12px; }
    .view-switch { margin-left: 0; margin-top: 10px; }
}
.store-card {
    background: #ffffff85;
...
.store-card-prices .price-tag .label {
    color: #fff;
    padding: 1px 3px;
    border-radius: 3px;
    margin-right: 3px;
...
.buy-modal-skin .layui-layer-btn { 
    text-align: center !important; 
    padding: 15px 20px !important; 
    border-top: 1px solid #f0f0f0 !important;
}
.buy-modal-skin .layui-layer-btn a { 
...
.buy-modal-skin .layui-layer-btn .layui-layer-btn0 { 
    background: #16baaa !important; 
    color: #fff !important;
}
.buy-modal-skin .layui-layer-btn .layui-layer-btn0:hover { 
    background: #139d91 !important;
}
.buy-modal-skin .layui-layer-btn .layui-layer-btn1 { 
    background: #f0f0f0 !important; 
    color: #666 !important;
}
.buy-modal-skin .layui-layer-btn .layui-layer-btn1:hover {
...
.buy-trial-section.tried .trial-tip-text {
    color: #999;
    justify-content: center;
    width: 100%;
}
...
.buy-trial-section.tried .trial-tip-text i {
    color: #999;
}
.trial-btn {
    background: #9333ea;
...
/* 深色模式试用样式 */
.dark-mode .buy-trial-section {
    background: #2d1f3d !important;
    border-color: #6b21a8 !important;
}
.dark-mode .buy-trial-section.tried {
    background: #2a2a2a !important;
    border-color: #444 !important;
}
.dark-mode .trial-tip-text {
    color: #c4b5fd !important;
}
.dark-mode .buy-trial-section.tried .trial-tip-text {
    color: #888 !important;
}
...
<!-- 顶部导航 -->
<?php
$storeTemplateTypeForView = $template_type ?? 'all';
$storeTemplateTabMap = [
    'template'            => 'home',
    'user_template'       => 'user',
...
$installedTemplateHref = './template.php' . (isset($storeTemplateTabMap[$storeTemplateTypeForView]) ? '?tab=' . $storeTemplateTabMap[$storeTemplateTypeForView] : '');
$templateStoreHref = './store.php?action=tpl' . ($storeTemplateTypeForView !== 'all' ? '&template_type=' . urlencode($storeTemplateTypeForView) : '');
?>
<div class="layui-tabs order-tabs-wrapper" style="display:flex;align-items:center;justify-content:space-between;" lay-options="{trigger: false}">
    <ul class="layui-tabs-header">
        ...
        <li class="layui-this"><a href="<?= htmlspecialchars($templateStoreHref, ENT_QUOTES) ?>">模板主题</a></li>
        <li><a href="./store.php?action=plu">扩展插件</a></li>
        <li><a href="./store.php?action=purchased">已购买</a></li>
        <li><a href="<?= htmlspecialchars($installedTemplateHref, ENT_QUOTES) ?>">已安装</a></li>
    </ul>
    <div style="display:flex;align-items:center;gap:6px;flex-shrink:0;">
        <div class="layui-input-inline layui-input-wrap" style="width:180px;margin:0;">
            <input type="text" class="layui-input" id="store-search" placeholder="搜索模板..." lay-affix="clear">
        </div>
...
<!-- 分类标签 -->
<?php if (!empty($tpl_type_arr) && count($tpl_type_arr) > 1): ?>
<div class="store-category-bar">
    <div class="category-tabs">
        <?php foreach ($tpl_type_arr as $cat): ?>
        <span class="tab-item <?= $tpl_type == $cat['id'] ? 'active' : '' ?>" data-id="<?= $cat['id'] ?>"><?= htmlspecialchars($cat['title']) ?></span>
        <?php endforeach; ?>
    </div>
</div>
<?php else: ?>
<div class="store-category-bar" style="margin-bottom:12px;">
</div>
<?php endif; ?>
...
<div class="layui-table-view" style="border-radius:8px;overflow:hidden;">
    <div class="layui-card-header" style="display:flex;align-items:center;justify-content:center;position:relative;height:auto;padding:12px 15px;">
        ...
        <span style="color:#667797;font-size:14px;font-weight:500;">模板主题</span>
...
<script type="text/html" id="cover">
    <a href="javascript:;" lay-event="img">
        {{# if(d.cover){ }}
        <img onerror="this.onerror=null; this.parentNode.innerHTML='<i class=\'ri-layout-4-line\' style=\'font-size:24px;color:#999;\'></i>'" src="{{ d.cover }}" style="width: 45px; height: 45px; border-radius: 6px; object-fit: cover;" />
        {{# } else { }}
        <i class="ri-layout-4-line" style="font-size:24px;color:#999;"></i>
        {{# } }}
    </a>
</script>
...
<script type="text/html" id="name">
    <div style="cursor: pointer;" lay-event="detail">
        <div><strong>{{ d.name }}</strong>{{# if(d.is_install == 'y'){ }}<span style="background:#16a34a;color:#fff;font-size:10px;padding:1px 5px;border-radius:3px;margin-left:6px;">已安装</span>{{# } else if(d.is_buy == 'y' && d.buy_type == 'trial'){ }}<span style="background:#9333ea;color:#fff;font-size:10px;padding:1px 5px;border-radius:3px;margin-left:6px;">试用中</span>{{# } else if(d.is_buy == 'y'){ }}<span style="background:#1e9fff;color:#fff;font-size:10px;padding:1px 5px;border-radius:3px;margin-left:6px;">已购买</span>{{# } }}{{# if(d.allow_buyout == 1 && (d.my_price > 0 || d.vip_price > 0 || d.svip_price > 0 || d.price_supreme > 0)){ }}<span style="background:#ff9800;color:#fff;font-size:10px;padding:1px 5px;border-radius:3px;margin-left:6px;">可买断</span>{{# } }}{{# if(d.trial_days > 0 && d.can_trial == 1){ }}<span style="background:#9333ea;color:#fff;font-size:10px;padding:1px 5px;border-radius:3px;margin-left:6px;">可试用{{ d.trial_days }}天</span>{{# } }}</div>
...
<script type="text/html" id="vip">
    {{# if(d.has_monthly && d.price_monthly_vip > 0){ }}
    <span style="color: #ea580c; font-weight: 600; font-size: 13px;">¥{{ d.price_monthly_vip }}<span style="font-size:11px;color:#999;">/月</span></span>
    {{# } else if(d.vip_price == 0){ }}
    <span style="color: #16a34a; font-weight: 600; font-size: 14px;">免费</span>
    {{# } else { }}
    <span style="color: #ea580c; font-weight: 600; font-size: 14px;">¥{{ d.vip_price }}</span>
    {{# } }}
</script>

<script type="text/html" id="svip">
    {{# if(d.has_monthly && d.price_monthly_svip > 0){ }}
    <span style="color: #ea580c; font-weight: 600; font-size: 13px;">¥{{ d.price_monthly_svip }}<span style="font-size:11px;color:#999;">/月</span></span>
    {{# } else if(d.svip_price == 0){ }}
    <span style="color: #16a34a; font-weight: 600; font-size: 14px;">免费</span>
    {{# } else { }}
    <span style="color: #ea580c; font-weight: 600; font-size: 14px;">¥{{ d.svip_price }}</span>
    {{# } }}
</script>

<script type="text/html" id="zhizun">
    {{# if(d.has_monthly && d.price_monthly_supreme > 0){ }}
    <span style="color: #ea580c; font-weight: 600; font-size: 13px;">¥{{ d.price_monthly_supreme }}<span style="font-size:11px;color:#999;">/月</span></span>
    {{# } else { }}
    <span style="color: #16a34a; font-weight: 600; font-size: 14px;">免费</span>
    {{# } }}
</script>
...
<script type="text/html" id="operate">
    {{# if(d.is_install == 'y'){ }}
    {{# if(d.is_buy == 'y' && d.buy_type == 'permanent'){ }}
    <button class="layui-btn layui-btn-sm layui-btn-disabled">已买断</button>
    {{# } else if(d.is_buy == 'y' && d.buy_type == 'trial'){ }}
    <button class="layui-btn layui-btn-sm" lay-event="showExpire">试用期</button>
    <button class="layui-btn layui-btn-sm layui-btn-warm" lay-event="buy">购买</button>
    {{# } else if(d.is_buy == 'y' && d.buy_type == 'monthly'){ }}
    ...
    <button class="layui-btn layui-btn-sm layui-btn-warm" lay-event="renew">续期</button>
    {{# } else if(!d.reg_type){ }}
    <button class="layui-btn layui-btn-sm layui-btn-normal" lay-event="auth">需授权</button>
    {{# } else if(d.is_buy == 'y'){ }}
    <button class="layui-btn layui-btn-sm" lay-event="install">安装</button>
    {{# } else if(d.my_price == 0 && d.my_price_monthly == 0){ }}
    <button class="layui-btn layui-btn-sm" lay-event="buy">获取</button>
    {{# } else { }}
    <button class="layui-btn layui-btn-sm layui-btn-warm" lay-event="buy">购买</button>
    {{# } }}
</script>
...
<script>
var storeData = [];
var allStoreData = []; // 保存当前页数据
var storeTotalCount = 0;
var currentKeyword = '';
...
var currentTemplateType = '<?= htmlspecialchars($template_type ?? 'all', ENT_QUOTES) ?>';

function renderStoreCards(pageData) {
    var dataToRender = pageData || storeData;
    if (dataToRender.length === 0 && allStoreData.length === 0) {
        $('#cards-container').removeClass('active').css({'display':'flex','justify-content':'center','align-items':'center','min-height':'300px'}).html('<div style="text-align:center;padding:50px 0;color:#999;"><i class="ri-layout-4-line" style="font-size:48px;display:block;margin-bottom:10px;"></i>暂无模板</div>');
    ...
    dataToRender.forEach(function(d) {
        var coverImg = d.cover ? '<img src="' + d.cover + '" onerror="this.parentNode.innerHTML=\'<i class=\\\'ri-layout-4-line default-icon\\\'></i>\'">' : '<i class="ri-layout-4-line default-icon"></i>';
        
        var installedTag = '';
        if (d.is_install === 'y') {
            installedTag = '<span class="store-card-installed"><i class="ri-check-line"></i> 已安装</span>';
        } else if (d.is_buy === 'y' && d.buy_type === 'trial') {
            installedTag = '<span class="store-card-installed" style="background:rgba(147,51,234,0.9);"><i class="ri-gift-line"></i> 试用中</span>';
        } else if (d.is_buy === 'y') {
            installedTag = '<span class="store-card-installed" style="background:rgba(30,159,255,0.9);"><i class="ri-shopping-cart-line"></i> 已购买</span>';
        }
        var buyoutTag = '';
        if (d.allow_buyout == 1 && (d.my_price > 0 || d.vip_price > 0 || d.svip_price > 0 || d.price_supreme > 0)) {
            buyoutTag = '<span class="store-card-installed" style="background:rgba(255,152,0,0.9);top:40px;"><i class="ri-vip-crown-line"></i> 可买断</span>';
        }
        if (d.trial_days > 0 && d.can_trial == 1) {
            var trialTop = buyoutTag ? '70px' : '40px';
            buyoutTag += '<span class="store-card-installed" style="background:rgba(147,51,234,0.9);top:' + trialTop + ';"><i class="ri-gift-line"></i> 可试用' + d.trial_days + '天</span>';
        }
        
        // 价格显示 - 优先显示月付价格
        var pricesHtml = '<div class="store-card-prices">';
        if (d.has_monthly) {
            pricesHtml += '<span class="price-tag vip"><span class="label">VIP</span><span class="value' + (d.price_monthly_vip == 0 ? ' free' : '') + '">' + (d.price_monthly_vip == 0 ? '免费' : '¥' + d.price_monthly_vip + '/月') + '</span></span>';
            pricesHtml += '<span class="price-tag svip"><span class="label">SVIP</span><span class="value' + (d.price_monthly_svip == 0 ? ' free' : '') + '">' + (d.price_monthly_svip == 0 ? '免费' : '¥' + d.price_monthly_svip + '/月') + '</span></span>';
            pricesHtml += '<span class="price-tag zhizun"><span class="label">至尊</span><span class="value' + (d.price_monthly_supreme == 0 ? ' free' : '') + '">' + (d.price_monthly_supreme == 0 ? '免费' : '¥' + d.price_monthly_supreme + '/月') + '</span></span>';
        } else {
            pricesHtml += '<span class="price-tag vip"><span class="label">VIP</span><span class="value' + (d.vip_price == 0 ? ' free' : '') + '">' + (d.vip_price == 0 ? '免费' : '¥' + d.vip_price) + '</span></span>';
            pricesHtml += '<span class="price-tag svip"><span class="label">SVIP</span><span class="value' + (d.svip_price == 0 ? ' free' : '') + '">' + (d.svip_price == 0 ? '免费' : '¥' + d.svip_price) + '</span></span>';
            pricesHtml += '<span class="price-tag zhizun"><span class="label">至尊</span><span class="value' + (d.price_supreme == 0 ? ' free' : '') + '">' + (d.price_supreme == 0 ? '免费' : '¥' + d.price_supreme) + '</span></span>';
        }
        pricesHtml += '</div>';
        
        var actionBtn = '';
        var isUnauthorized = !d.reg_type;
        if (d.is_install === 'y') {
            if (d.is_buy === 'y' && d.buy_type === 'permanent') {
                actionBtn = '<button class="layui-btn layui-btn-sm layui-btn-disabled">已买断</button>';
            } else if (d.is_buy === 'y' && d.buy_type === 'trial') {
                actionBtn = '<button class="layui-btn layui-btn-sm" data-event="showExpire">试用期</button>';
                actionBtn += '<button class="layui-btn layui-btn-sm layui-btn-warm" data-event="buy">购买</button>';
            } else if (d.is_buy === 'y' && d.buy_type === 'monthly') {
                actionBtn = '<button class="layui-btn layui-btn-sm" data-event="showExpire">期限</button>';
                actionBtn += '<button class="layui-btn layui-btn-sm layui-btn-warm" data-event="renew">续期</button>';
            } else {
                actionBtn = '<button class="layui-btn layui-btn-sm layui-btn-danger layui-btn-disabled">已到期</button>';
                actionBtn += '<button class="layui-btn layui-btn-sm layui-btn-warm" data-event="renew">续期</button>';
            }
        } else if (d.is_expired == 1) {
            actionBtn = '<button class="layui-btn layui-btn-sm layui-btn-danger layui-btn-disabled">已到期</button>';
        ...
        else if (isUnauthorized) actionBtn = '<button class="layui-btn layui-btn-sm layui-btn-normal" data-event="auth">需授权</button>';
        else if (d.is_buy === 'y') actionBtn = '<button class="layui-btn layui-btn-sm" data-event="install">安装</button>';
        else if (d.my_price == 0 && d.my_price_monthly == 0) actionBtn = '<button class="layui-btn layui-btn-sm" data-event="buy">获取</button>';
        else actionBtn = '<button class="layui-btn layui-btn-sm layui-btn-warm" data-event="buy">购买</button>';
        
        html += '<div class="store-card" data-app=\'' + JSON.stringify(d).replace(/'/g, "&#39;") + '\'>';
        html += '  <div class="store-card-cover" data-event="img">' + coverImg + installedTag + buyoutTag + '</div>';
        ...
        html += '    <div class="store-card-desc">' + (d.description || '暂无描述') + '</div>' + pricesHtml + '</div>';
...
function loadStoreData(callback, page, limit) {
    page = page || cardCurrentPage || 1;
    limit = limit || cardPageSize || 10;
    var loadIndex = layer.load(2);
    $.ajax({
        url: '?action=tpl_ajax&tpl_type=' + currentCategory + '&template_type=' + encodeURIComponent(currentTemplateType) + '&keyword=' + encodeURIComponent(currentKeyword) + '&page=' + page + '&limit=' + limit, type: 'GET', dataType: 'json',
        success: function(res) { 
            allStoreData = res.data || []; 
            storeData = allStoreData;
            storeTotalCount = parseInt(res.count || allStoreData.length || 0, 10);
...
function renderCardPagination() {
    if (allStoreData.length === 0) {
        // 显示空数据提示
        $('#cards-container').removeClass('active').css({'display':'flex','justify-content':'center','align-items':'center','min-height':'300px'}).html('<div style="text-align:center;padding:50px 0;color:#999;"><i class="ri-layout-4-line" style="font-size:48px;display:block;margin-bottom:10px;"></i>该分类下暂无模板</div>');
        $('#cards-pagination').hide();
        return;
    }
    layui.use('laypage', function(){
        ...
        laypage.render({
            ...
            jump: function(obj, first){
                var pageChanged = obj.curr !== cardCurrentPage || obj.limit !== cardPageSize;
                cardCurrentPage = obj.curr;
                cardPageSize = obj.limit;
                if (!first && pageChanged) {
                    loadStoreData(function(){ renderCardPagination(); }, obj.curr, obj.limit);
                    return;
                }
                renderStoreCards(allStoreData);
            }
        });
    });
...
var tableInstance = null; // 全局table实例，供详情弹窗按钮使用
var currentCategory = <?= (int)$tpl_type ?>; // 当前分类
...
layui.use(['table'], function(){
    ...
    table.render({
        elem: '#index', url: '?action=tpl_ajax', page: true, limits: [10, 20, 50], lineStyle: 'height: 65px;',
        where: { tpl_type: currentCategory, template_type: currentTemplateType },
        text: { none: '该分类下暂无模板' },
        cols: [[
            {field: 'cover', title: '图标', width: 75, templet: '#cover', align: 'center'},
            {field: 'name', title: '模板名称', templet: '#name'},
            {field: 'vip_price', title: 'VIP', width: 80, align: 'center', templet: '#vip'},
            {field: 'svip_price', title: 'SVIP', width: 80, align: 'center', templet: '#svip'},
            {field: 'zhizun', title: '至尊', width: 80, align: 'center', templet: '#zhizun'},
            {field: 'author', title: '作者', width: 100, align: 'center'},
            {field: 'version', title: '版本', width: 90, align: 'center'},
            {fixed: 'right', title: '操作', width: 165, align: 'center', templet: '#operate'}
        ]],
        done: function(res) { if (res.data) storeData = res.data; }
    });
    ...
    $('.category-tabs .tab-item').on('click', function(){
        ...
        table.reload('index', { page: { curr: 1 }, where: { tpl_type: id, template_type: currentTemplateType } });
        if (currentView === 'card') {
            allStoreData = [];
            cardCurrentPage = 1;
            loadStoreData(function(){ renderCardPagination(); }, 1, cardPageSize);
        }
    });
    ...
    $('#store-reset-btn').on('click', function() {
        ...
        $('#store-search').val('');
        $('.category-tabs .tab-item').removeClass('active').first().addClass('active');
        currentCategory = 0;
        currentKeyword = '';
        table.reload('index', { page: { curr: 1 }, where: { tpl_type: 0, keyword: '', template_type: currentTemplateType } });
        if (currentView === 'card') {
            allStoreData = [];
            cardCurrentPage = 1;
            loadStoreData(function(){ renderCardPagination(); }, 1, cardPageSize);
    ...
    $('.view-btn').on('click', function() {
        ...
        if (view === 'card') {
            ...
            if (allStoreData.length === 0) loadStoreData(function(){ renderCardPagination(); }, cardCurrentPage, cardPageSize);
    ...
    function filterStoreTemplates(keyword) {
        // 使用API搜索（API支持keyword参数）
        if (currentView === 'card') {
            currentKeyword = keyword;
            cardCurrentPage = 1;
            loadStoreData(function(){ renderCardPagination(); }, 1, cardPageSize);
        } else {
            table.reload('index', { page: { curr: 1 }, where: { tpl_type: currentCategory, template_type: currentTemplateType, keyword: keyword } });
        }
    }
    ...
    // 从URL参数读取keyword，自动填充搜索框并触发搜索
    var urlKeyword = new URLSearchParams(window.location.search).get('keyword');
    if (urlKeyword) {
        $('#store-search').val(urlKeyword);
        setTimeout(function(){ filterStoreTemplates(urlKeyword); }, 300);
    }
});

function getTemplateInstallType(data) {
    var type = (data && (data.install_type || data.type)) ? String(data.install_type || data.type) : 'tpl';
    if (type === 'home_template' || type === 'station_template' || type === 'home' || type === 'station') type = 'template';
    ...
    var allow = ['tpl', 'template', 'user_template', 'bottom_nav_template', 'blog_template'];
    return allow.indexOf(type) >= 0 ? type : 'tpl';
}

function getTemplateManageUrl(data) {
    var type = getTemplateInstallType(data);
    var s = data.name ? '&search=' + encodeURIComponent(data.name) : '';
    if (type === 'user_template') return './template.php?tab=user' + s;
    if (type === 'bottom_nav_template') return './template.php?tab=bottom_nav' + s;
    if (type === 'blog_template') return './template.php?tab=blog' + s;
    return './template.php?tab=home' + s;
}
...
function handleAction(event, data, table) {
    // 如果没传table参数，使用全局tableInstance
    var tbl = table || tableInstance;
    
    // 刷新数据的辅助函数
    function refreshData() {
        if(tbl) tbl.reload('index');
        if (currentView === 'card') loadStoreData(function(){ renderCardPagination(); }, cardCurrentPage, cardPageSize);
    }
    
    if(event === 'blocked') {
        layer.msg('您已被开发者拉黑！', {icon: 2, time: 3000});
        return;
    }
    ...
    if(event === 'auth') layer.confirm('需要授权才能安装，是否前往授权？', {icon: 3}, function(){ location.href = './auth.php'; });
    if(event === 'install'){
        asyncInstallApp(data.id, getTemplateInstallType(data), data.name, function(){
            layer.open({
                type: 1, title: '安装成功', area: '400px',
                content: '<div style="padding:20px;text-align:center;"><i class="ri-checkbox-circle-fill" style="font-size:48px;color:#16a34a;"></i><div style="margin:15px 0;font-size:16px;">模板「' + data.name + '」已安装</div></div>',
                btn: ['去启用', '稍后再说'],
                yes: function(i) { layer.close(i); location.href = getTemplateManageUrl(data); },
                btn2: function(i) { layer.close(i); refreshData(); }
            });
        }, function(err){ layer.msg(err, {icon: 2}); });
    }
    if(event === 'buy'){
        data.install_type = getTemplateInstallType(data);
        showBuyModalTpl(data, tbl, refreshData);
    }
...
// 模板购买弹窗函数
function showBuyModalTpl(data, tbl, refreshCallback) {
    // has_monthly 表示应用是否支持月付（任一等级有月付价格即支持）
    ...
    if (isFree) {
            ...
            $.post('./store.php?action=buy', { app_id: data.id, buy_type: 'permanent', months: 1 }, function(res){
                ...
                if(res.code == 0) {
                    layer.open({
                        ...
                        content: '<div style="padding:20px;text-align:center;"><i class="ri-checkbox-circle-fill" style="font-size:48px;color:#16a34a;"></i><div style="margin:15px 0;font-size:16px;">模板「' + data.name + '」已添加到您的应用列表</div></div>',
                        btn: ['立即安装', '稍后再说'],
                        yes: function(idx) {
                            layer.close(idx);
                            asyncInstallApp(data.id, getTemplateInstallType(data), data.name, function(){
        ...
        // 试用选项
        if (data.can_trial == 1 && data.trial_days > 0) {
            buyOptionsHtml += '<div class="buy-trial-section">';
            buyOptionsHtml += '<div class="trial-tip-content">';
            buyOptionsHtml += '<div class="trial-tip-text"><i class="ri-gift-line"></i> 首次可免费试用 ' + data.trial_days + ' 天</div>';
            buyOptionsHtml += '<button type="button" class="trial-btn" onclick="startTrial(' + data.id + ', ' + data.trial_days + ')">免费试用</button>';
            buyOptionsHtml += '</div>';
            buyOptionsHtml += '</div>';
        } else if (data.has_tried == 1) {
            buyOptionsHtml += '<div class="buy-trial-section tried">';
            buyOptionsHtml += '<div class="trial-tip-text"><i class="ri-information-line"></i> 您已使用过试用机会</div>';
            buyOptionsHtml += '</div>';
        }
        ...
        // 试用功能
        window.startTrial = function(appId, trialDays) {
            layer.confirm('确定开始试用？试用期 ' + trialDays + ' 天，每个应用仅可试用一次。', {
                title: '免费试用',
                btn: ['开始试用', '取消']
            }, function(index) {
                layer.close(index);
                layer.load(2);
                $.post('./store.php?action=buy', { app_id: appId, buy_type: 'trial' }, function(res){
                    layer.closeAll();
                    if(res.code == 0) {
                        layer.msg('试用开通成功！', {icon: 1, time: 1500}, function(){
                            location.reload();
                        });
                    } else {
                        layer.msg(res.msg || '试用失败', {icon: 2});
                    }
                }, 'json').fail(function(xhr){
                    layer.closeAll();
                    var msg = '请求失败';
                ...
                $.post('./store.php?action=buy', { app_id: data.id, buy_type: 'permanent', months: 1 }, function(res){
                    ...
                    if(res.code == 0) {
                        layer.open({
                            ...
                            icon: 1,
                            area: '400px',
                            content: '<div style="padding:20px;text-align:center;"><i class="ri-checkbox-circle-fill" style="font-size:48px;color:#16a34a;"></i><div style="margin:15px 0;font-size:16px;">模板「' + data.name + '」已买断</div></div>',
                            btn: ['立即安装', '稍后再说'],
                            yes: function(idx) {
                                layer.close(idx);
                                asyncInstallApp(data.id, getTemplateInstallType(data), data.name, function(){
                        ...
                        if(res.data && res.data.need_recharge) layer.confirm(res.msg + '<br><br>是否前往授权中心充值？', {icon: 2, title: '余额不足'}, function(){ window.open('<?= $licenseCenterUrl ?>'); }); 
        ...
        window.setMonths = function(months, price) {
            ...
            $('.preset-btn').each(function() {
                if ($(this).text() === months + '个月' || (months === 12 && $(this).text() === '1年')) {
                    $(this).addClass('active');
                }
            });
        };
        ...
        layer.open({ 
            ...
            $.post('./store.php?action=buy', { app_id: data.id, buy_type: buyType, months: months }, function(res){
                ...
                var successText = buyType === 'monthly' ? '模板「' + data.name + '」已订阅' + months + '个月' : '模板「' + data.name + '」已购买';
                if(res.code == 0) {
                    layer.open({
                        type: 1,
                        title: successTitle,
                        ...
                        btn2: function(idx) { layer.close(idx); refreshCallback(); }
                    });
...
function showAppDetail(app) {
    ...
    else if (!app.reg_type) actionsHtml = '<button class="layui-btn layui-btn-normal" onclick="doDetailAction(\'auth\')" style="flex:1;"><i class="ri-shield-check-line"></i> 需授权</button>';
    else if (app.is_buy === 'y') actionsHtml = '<button class="layui-btn" onclick="doDetailAction(\'install\')" style="flex:1;"><i class="ri-download-line"></i> 安装</button>';
    else if (app.my_price == 0 && app.my_price_monthly == 0) actionsHtml = '<button class="layui-btn" onclick="doDetailAction(\'buy\')" style="flex:1;"><i class="ri-download-line"></i> 获取</button>';
    else actionsHtml = '<button class="layui-btn layui-btn-warm" onclick="doDetailAction(\'buy\')" style="flex:1;"><i class="ri-shopping-cart-line"></i> 购买</button>';
    ...
    } else if (app.is_buy === 'y' && app.buy_type === 'trial') {
    ...
    } else if (app.is_buy === 'y') {
        statusTags += '<span style="background:#1e9fff;color:#fff;font-size:10px;padding:2px 6px;border-radius:3px;margin-left:8px;">已购买</span>';
    }
    if (app.allow_buyout == 1 && (app.my_price > 0 || app.vip_price > 0 || app.svip_price > 0 || app.price_supreme > 0)) {
        statusTags += '<span style="background:#ff9800;color:#fff;font-size:10px;padding:2px 6px;border-radius:3px;margin-left:8px;">可买断</span>';
    }
    if (app.trial_days > 0 && app.can_trial == 1) {
        statusTags += '<span style="background:#9333ea;color:#fff;font-size:10px;padding:2px 6px;border-radius:3px;margin-left:8px;">可试用' + app.trial_days + '天</span>';
    }
    ...
    var html = '<div class="app-detail-modal" onclick="closeAppDetail()"><div class="app-detail-box" onclick="event.stopPropagation()"><div class="app-detail-header">' +
        ...
        '<div class="app-detail-meta"><span><i class="ri-palette-line"></i> 模板</span><span><i class="ri-price-tag-3-line"></i> v' + app.version + '</span><span><i class="ri-user-line"></i> ' + escapeHtml(app.author || '未知') + '</span></div>' +
