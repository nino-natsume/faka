...
class SkuTable {
    ...
    css() {
        $('head').append(`<style>
            .fairy-product-type-row {
              display: flex;
              align-items: center;
              gap: 8px;
              max-width: 560px;
            }
            .fairy-product-type-row .layui-form-select {
              flex: 1;
              min-width: 0;
            }
            .fairy-product-type-row .fairy-goods-type-create {
              flex: 0 0 auto;
              height: 38px;
              line-height: 38px;
              padding: 0 12px;
            }
            #${this.options.specTableElemId} tfoot .fairy-spec-create {
              display: inline-flex;
              align-items: center;
              gap: 4px;
              color: #1E9FFF;
    ...
    listen() {
         ...
         * 监听新建规格模板
         */
        $(document).on('click', `#${this.options.productTypeElemId}-create`, function () {
            if (!that.options.goodsTypeCreateUrl) return;
            layer.prompt({title: '新建规格模板', formType: 0}, function (value, index, elem) {
                value = $.trim(value || '');
                if (!value) {
                    layer.msg('请输入规格模板名称');
                    return;
                ...
                Util.request.post(
                    {url: that.options.goodsTypeCreateUrl, data: {title: value}},
                    function (res) {
                        that.data.productTypeId = res.data.id;
                        that.data.attributeData = [];
                        that.data.specData = [];
                        that.data.skuData = $.extend(that.data.skuData, that.getFormSkuData());
                        that.resetRender([that.options.productTypeElemId, that.options.attributeTableElemId, that.options.specTableElemId, that.options.skuTableElemId]);
                        that.renderProductType();
                        layer.msg('已创建规格模板：' + value);
                    });
                Util.msg.close(index);
            });
        });
            ...
            Util.msg.prompt({title: $(thisI).parent().text().trim() + '批量赋值'}, function (value, index, elem) {
                $.each($(`#${that.options.skuTableElemId} tbody tr`), function () {
                    var index = that.options.rowspan ?
                        ...
                        $(thisI).parent().index();
                    var $inp = $(this).find('td').eq(index).children('input');
                    $inp.val(value);
                    SkuTable.applyPriceStyle($inp);
                });
                Util.msg.close(index);
            });
        });
        ...
        $(document).on('click', `#${this.options.specTableElemId} .fairy-spec-create`, function () {
            ...
            layer.prompt({title: '添加规格名', formType: 0}, function (value, index, elem) {
                value = $.trim(value || '');
                if (!value) {
                    layer.msg('请输入规格名称');
                    return;
        ...
        $(document).on('click', `#${this.options.specTableElemId} .fairy-spec-value-create`, function () {
            ...
            layer.prompt({title: '添加规格值', formType: 0}, function (value, index, elem) {
                value = $.trim(value || '');
                if (!value) {
                    layer.msg('请输入规格值');
                    return;
                    ...
                    function (res) {
                        that.data.specData.forEach(function (v, i) {
                            if (v.id == specId) {
                                ...
                                if (v.value.indexOf(res.data.id) === -1 && v.value.indexOf(String(res.data.id)) === -1) {
                                    v.value.push(res.data.id);
                                }
                            }
                        });
                        that.data.skuData = $.extend(that.data.skuData, that.getFormSkuData());
    ...
    renderProductType() {
        Util.request.get({url: this.options.productTypeUrl}, (res) => {
            ...
            html += '<div class="fairy-product-type-row">';
            ...
            html += '</select>';
            if (this.options.goodsTypeCreateUrl) {
                html += `<button type="button" class="layui-btn layui-btn-primary layui-btn-sm fairy-goods-type-create" id="${this.options.productTypeElemId}-create"><i class="layui-icon layui-icon-add-1"></i> 新建模板</button>`;
            }
            html += '</div><div class="layui-form-mid layui-text-em">可在本页直接新建模板、添加规格名和规格值，无需跳转到商品规格页面。</div></div></div>';
    ...
    renderSpecTable() {
        ...
        $.each(this.data.specData, function (index, item) {
            ...
            table += '</div>';
            that.options.specValueCreateUrl && (table += '<div class="fairy-spec-value-create"><i class="layui-icon layui-icon-addition"></i> 添加规格值</div>');
            table += '</td>';
            table += '</tr>';
        });
        table += '</tbody>';
        ...
        if (this.options.specCreateUrl) {
            table += `<div class="fairy-spec-create"><i class="layui-icon layui-icon-addition"></i> 添加规格名</div>`;
        }
        table += '</td></tr></tfoot>';
    ...
    renderSingleSkuTable() {
        ...
        this.options.singleSkuTableConfig.thead.forEach((item) => {
            table += `<th>${item.title}${item.icon ? ' <i class="' + item.icon + '" style="color:#1e9fff !important;cursor:pointer;"></i>' : ''}</th>`;
        });
        table += '</tr>';
        table += '</thead>';
        ...
        that.options.singleSkuTableConfig.tbody.forEach(function (item) {
            switch (item.type) {
                ...
                default:
                    table += '<td>';
                    table += `<input type="text" name="${item.field}" value="${that.data.skuData[item.field] ? that.data.skuData[item.field] : item.value}" class="layui-input" placeholder="${item.placeholder || ''}" lay-verify="${item.verify}" lay-reqtext="${item.reqtext}">`;
                    table += '</td>';
                    break;
            }
        });
        ...
        if(this.options.operate != 'stock'){
            ...
            SkuTable.applyAllPriceStyles('#' + this.options.skuTableElemId);
        }
        ...
        if ($(`#${this.options.specTableElemId} tbody input[type=checkbox]:checked`).length) {
            ...
            if (prependThead.length > 0) {
                ...
                this.options.multipleSkuTableConfig.thead.forEach(function (item) {
                    theadTr += '<th style="width: 200px;">' + item.title + (item.icon ? ' <i class="' + item.icon + '" style="color:#1e9fff !important;cursor:pointer;"></i>' : '') + '</th>';
                });
                theadTr += '</tr>';
                ...
                that.options.multipleSkuTableConfig.tbody.forEach(function (c) {
                    ...
                    switch (c.type) {
                        ...
                        case "sdk":
                            ...
                            break;
                        case "number":
                            tr += '<td><input type="number" name="' + that.makeSkuName(item, c) + '" value="' + (that.data.skuData[that.makeSkuName(item, c)] !== undefined ? that.data.skuData[that.makeSkuName(item, c)] : '') + '" class="layui-input" placeholder="' + (c.placeholder || '') + '" lay-verify="' + c.verify + '" lay-reqtext="' + c.reqtext + '" /></td>';
                            break;
                        case "input":
                        default:
                            tr += '<td><input type="text" name="' + that.makeSkuName(item, c) + '" value="' + (that.data.skuData[that.makeSkuName(item, c)] !== undefined ? that.data.skuData[that.makeSkuName(item, c)] : '') + '" class="layui-input" placeholder="' + (c.placeholder || '') + '" lay-verify="' + c.verify + '" lay-reqtext="' + c.reqtext + '" /></td>';
                            break;
                    }
                });
                tr += '</tr>';
...
// 价格输入框：有值时红色加粗，无值时恢复默认
SkuTable.applyPriceStyle = function(input) {
    var $el = $(input);
    var val = parseFloat($el.val());
    if (val > 0) {
        $el.css({'color': '#eb2525', 'font-weight': 'bold'});
    } else {
...
// 批量设置价格输入框样式
SkuTable.applyAllPriceStyles = function(container) {
    $(container).find('input[type="text"], input[type="number"]').each(function(){
        SkuTable.applyPriceStyle(this);
    });
}

// 价格输入框实时红色加粗
$(document).on('input change', '#fairy-sku-table input[type="text"], #fairy-sku-table input[type="number"]', function(){
    SkuTable.applyPriceStyle(this);
});
