...
.app-detail-prices .price-item.zhizun { background: #efe8e5; }
...
@media (max-width: 768px) {
    ...
    .view-switch { margin-left: 0; margin-top: 10px; width: 100%; justify-content: flex-end; }
    .store-cards.active {
    ...
    .store-card-desc { font-size: 12px; height: 36px; }
    .store-card-footer { padding: 10px 12px; }
...
<div class="layui-tabs order-tabs-wrapper" style="display:flex;align-items:center;justify-content:space-between;" lay-options="{trigger: false}">
    <ul class="layui-tabs-header">
        <li class="layui-this"><a href="./store.php">全部应用</a></li>
    ...
    <div style="display:flex;align-items:center;gap:6px;flex-shrink:0;">
        <div class="layui-input-inline layui-input-wrap" style="width:180px;margin:0;">
            <input type="text" class="layui-input" id="store-search" placeholder="搜索应用..." lay-affix="clear">
        </div>
...
<div class="store-category-bar">
    <div class="category-tabs">
        <span class="tab-item active" data-id="0" data-type="all">全部</span>
        <?php foreach ($plugin_type_arr as $cat): if($cat['id'] == 0) continue; ?>
        <span class="tab-item" data-id="<?= $cat['id'] ?>" data-type="plugin"><?= htmlspecialchars($cat['title']) ?></span>
        <?php endforeach; ?>
    </div>
</div>
...
<div class="layui-table-view" style="border-radius:8px;overflow:hidden;">
    <div class="layui-card-header" style="display:flex;align-items:center;justify-content:center;position:relative;height:auto;padding:12px 15px;">
        ...
        <span style="color:#667797;font-size:14px;font-weight:500;">全部应用</span>
...
function renderStoreCards(pageData) {
    ...
    if (dataToRender.length === 0 && allStoreData.length === 0) {
        $('#cards-container').removeClass('active').css({'display':'flex','justify-content':'center','align-items':'center','min-height':'300px'}).html('<div style="text-align:center;padding:50px 0;color:#999;"><i class="ri-apps-line" style="font-size:48px;display:block;margin-bottom:10px;"></i>暂无应用</div>');
    ...
    dataToRender.forEach(function(d) {
        ...
        if (d.is_install === 'y') {
            ...
            actionBtn = '<button class="layui-btn layui-btn-sm layui-btn-normal" data-event="auth">需授权</button>';
        } else if (d.is_buy === 'y') {
            actionBtn = '<button class="layui-btn layui-btn-sm" data-event="install">安装</button>';
        } else if (d.my_price == 0 && d.my_price_monthly == 0) {
            actionBtn = '<button class="layui-btn layui-btn-sm" data-event="buy">获取</button>';
        } else {
            actionBtn = '<button class="layui-btn layui-btn-sm layui-btn-warm" data-event="buy">购买</button>';
        }
        ...
        html += '      <div class="store-card-title">' + d.name + '</div>';
        html += '      <span class="store-card-version">v' + d.version + '</span>';
        html += '    </div>';
        html += '    <div class="store-card-desc">' + (d.description || '暂无描述') + '</div>';
        html += '    ' + pricesHtml;
        html += '  </div>';
        html += '  <div class="store-card-meta"><div class="meta-left"><span><i class="ri-user-line"></i>' + (d.author || '未知') + '</span></div><div class="store-card-actions">' + actionBtn + '</div></div>';
        html += '</div>';
    });
...
function buildStoreRequestUrl(page, limit) {
    page = page || cardCurrentPage || 1;
    limit = limit || cardPageSize || 10;
    var url = '?action=index';
    if (currentType === 'template') {
        url = '?action=tpl_ajax';
    } else if (currentType === 'plugin') {
        url = '?action=plu_ajax&plugin_type=' + currentCategory;
    }
    url += (url.indexOf('?') === -1 ? '?' : '&') + 'keyword=' + encodeURIComponent(currentKeyword) + '&page=' + page + '&limit=' + limit;
    return url;
}

// 加载商店数据
function loadStoreData(url, callback, page, limit) {
    page = page || cardCurrentPage || 1;
    limit = limit || cardPageSize || 10;
    var loadIndex = layer.load(2);
    var requestUrl = url || buildStoreRequestUrl(page, limit);
    if (requestUrl.indexOf('limit=') === -1) {
        requestUrl += (requestUrl.indexOf('?') === -1 ? '?' : '&') + 'page=' + page + '&limit=' + limit;
    }
    if (requestUrl.indexOf('keyword=') === -1) {
...
function renderCardPagination() {
    if (allStoreData.length === 0) {
        // 显示空数据提示
        $('#cards-container').removeClass('active').css({'display':'flex','justify-content':'center','align-items':'center','min-height':'300px'}).html('<div style="text-align:center;padding:50px 0;color:#999;"><i class="ri-apps-line" style="font-size:48px;display:block;margin-bottom:10px;"></i>该分类下暂无应用</div>');
        $('#cards-pagination').hide();
        return;
    }
        ...
        laypage.render({
            ...
            jump: function(obj, first){
                ...
                if (!first && pageChanged) {
                    loadStoreData(null, function(){ renderCardPagination(); }, obj.curr, obj.limit);
                    return;
                }
...
function filterStoreApps(keyword) {
    ...
    if (currentView === 'card') {
        ...
        cardCurrentPage = 1;
        loadStoreData(null, function(){ renderCardPagination(); }, 1, cardPageSize);
    } else {
        tableInstance.reload('index', { page: { curr: 1 }, where: { keyword: keyword } });
    }
}
...
layui.use(['table'], function(){
    ...
    $('.category-tabs .tab-item').on('click', function(){
        ...
        if(type === 'template') {
            ...
            url = '?action=plu_ajax&plugin_type=' + id;
        }
    ...
    table.render({
        ...
        cols: [[
            ...
            {field: 'name', title: '应用名称', minWidth: 350, templet: '#name'},
    ...
    table.on('tool(index)', function(obj){
        handleStoreAction(obj.event, obj.data, table);
    });
    ...
    $('#store-reset-btn').on('click', function() {
        ...
        currentKeyword = '';
        cardCurrentPage = 1;
        table.reload('index', { page: { curr: 1 }, url: '?action=index', where: { keyword: '' } });
        if (currentView === 'card') loadStoreData(null, function(){ renderCardPagination(); }, 1, cardPageSize);
    });
    ...
    $('.view-btn').on('click', function() {
        ...
        if (view === 'card') {
            ...
            if (allStoreData.length === 0) {
                var url = '?action=index';
                if (currentType === 'template') url = '?action=tpl_ajax';
                else if (currentType === 'plugin') url = '?action=plu_ajax&plugin_type=' + currentCategory;
                loadStoreData(null, function(){ renderCardPagination(); }, cardCurrentPage, cardPageSize);
            } else {
    ...
    $('#cards-container').on('click', '[data-event]', function(e) {
        ...
        var card = $(this).closest('.store-card');
        var data = card.data('app');
        handleStoreAction(event, data, table);
    });
...
function getStoreInstallType(data) {
...
function isStoreTemplateType(data) {
    return getStoreInstallType(data) !== 'plugin';
}

function getStoreTypeName(data) {
    var type = getStoreInstallType(data);
    var map = {template:'模板', user_template:'用户后台模板', bottom_nav_template:'底部导航模板', blog_template:'博客模板'};
    return type === 'plugin' ? '插件' : (map[type] || '模板');
}

function getStoreManageUrl(data) {
    var type = getStoreInstallType(data);
...
// 统一处理商店操作
function handleStoreAction(event, data, table) {
    var tbl = table || window.tableInstance;
    if(event === 'blocked') {
    ...
    if(event === 'auth'){
        layer.confirm('需要授权才能安装，是否前往授权？', {icon: 3}, function(){
            location.href = './auth.php';
        });
    }
    if(event === 'install'){
        var type = getStoreInstallType(data);
        var typeName = getStoreTypeName(data);
        var targetPage = getStoreManageUrl(data);
        asyncInstallApp(data.id, type, data.name, function(){
            layer.open({
                type: 1, title: '安装成功', area: '400px',
                content: '<div style="padding:20px;text-align:center;"><i class="ri-checkbox-circle-fill" style="font-size:48px;color:#16a34a;"></i><div style="margin:15px 0;font-size:16px;">' + typeName + '「' + data.name + '」已安装</div></div>',
                btn: ['去启用', '稍后再说'],
                yes: function(i) { layer.close(i); location.href = targetPage; },
                btn2: function(i) { layer.close(i); tbl.reload('index'); if (currentView === 'card') loadStoreData(null, function(){ renderCardPagination(); }); }
            });
    ...
    if(event === 'buy'){
        var typeName = getStoreTypeName(data);
        var targetPage = getStoreManageUrl(data);
        var installType = getStoreInstallType(data);
            ...
            $.post('./store.php?action=buy', { app_id: data.id, buy_type: 'permanent', months: 1 }, function(res){
                ...
                if(res.code == 0) {
                    layer.open({
                        ...
                        content: '<div style="padding:20px;text-align:center;"><i class="ri-checkbox-circle-fill" style="font-size:48px;color:#16a34a;"></i><div style="margin:15px 0;font-size:16px;">' + typeName + '「' + data.name + '」已添加到您的应用列表</div></div>',
                        btn: ['立即安装', '稍后再说'],
                        yes: function(idx) {
                            layer.close(idx);
                            asyncInstallApp(data.id, installType, data.name, function(){
                                layer.open({
                                    type: 1, title: '安装成功', area: '400px',
                                    content: '<div style="padding:20px;text-align:center;"><i class="ri-checkbox-circle-fill" style="font-size:48px;color:#16a34a;"></i><div style="margin:15px 0;font-size:16px;">' + typeName + '「' + data.name + '」已安装</div></div>',
                                    btn: ['去启用', '稍后再说'],
                                    yes: function(i) { layer.close(i); location.href = targetPage; },
                                    btn2: function(i) { layer.close(i); tbl.reload('index'); if (currentView === 'card') loadStoreData(null, function(){ renderCardPagination(); }, cardCurrentPage, cardPageSize); }
                                });
                            }, function(err){ layer.msg(err, {icon: 2}); });
                        },
                        btn2: function(idx) { layer.close(idx); tbl.reload('index'); if (currentView === 'card') loadStoreData(null, function(){ renderCardPagination(); }, cardCurrentPage, cardPageSize); }
                    });
                } else {
        ...
        window.switchToBuyout = function() {
            ...
            buyoutHtml += '</div>';
            if (hasMonthly) {
                buyoutHtml += '<div style="text-align: center; margin-bottom: 15px;"><a href="javascript:;" onclick="layer.closeAll();handleStoreAction(\'buy\', window.currentBuyData);" style="color: #1e9fff; font-size: 13px;"><i class="ri-arrow-left-line"></i> 返回月付</a></div>';
            }
                ...
                $.post('./store.php?action=buy', { app_id: data.id, buy_type: 'permanent', months: 1 }, function(res){
                    ...
                    if(res.code == 0) {
                        layer.open({
                            ...
                            title: '购买成功',
                            area: '400px',
                            content: '<div style="padding:20px;text-align:center;"><i class="ri-checkbox-circle-fill" style="font-size:48px;color:#16a34a;"></i><div style="margin:15px 0;font-size:16px;">' + typeName + '「' + data.name + '」已买断</div></div>',
                            btn: ['立即安装', '稍后再说'],
                            yes: function(idx) {
                                layer.close(idx);
                                asyncInstallApp(data.id, installType, data.name, function(){
        ...
        layer.open({ 
            ...
            $.post('./store.php?action=buy', { app_id: data.id, buy_type: buyType, months: months }, function(res){
                ...
                var successText = buyType === 'monthly' ? typeName + '「' + data.name + '」已订阅' + months + '个月' : typeName + '「' + data.name + '」已购买';
                if(res.code == 0) {
                    layer.open({
...
function showAppDetail(app) {
    ...
    var html = '<div class="app-detail-modal" onclick="closeAppDetail()"><div class="app-detail-box" onclick="event.stopPropagation()"><div class="app-detail-header">' +
        (app.cover ? '<img src="' + escapeHtml(app.cover) + '" onerror="this.style.display=\'none\'">' : '') + '<button class="app-detail-close" onclick="closeAppDetail()"><i class="ri-close-line"></i></button></div>' +
        '<div class="app-detail-body"><div class="app-detail-title">' + escapeHtml(app.name) + statusTags + '</div>' +
        '<div class="app-detail-meta"><span><i class="ri-' + (isStoreTemplateType(app) ? 'palette' : 'plug') + '-line"></i> ' + (getStoreTypeName(app)) + '</span><span><i class="ri-price-tag-3-line"></i> v' + app.version + '</span><span><i class="ri-user-line"></i> ' + escapeHtml(app.author || '未知') + '</span></div>' +
...
function doDetailAction(action) { var app = window.currentDetailApp; closeAppDetail(); if(app) handleStoreAction(action, app); }
