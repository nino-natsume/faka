...
if ($action === 'backup') {
    ...
    $bakfname = 'dcshop_' . date('Ymd') . '_' . substr(md5(AUTH_KEY . uniqid('', true)), 0, 18);
    $filename = '';
    $sqldump = '';
    ...
    $dumpfile = '#version:dcshop ' . Option::DC_VERSION . "\n";
    $dumpfile .= '#date:' . date('Y-m-d H:i') . "\n";
    $dumpfile .= '#tableprefix:' . DB_PREFIX . "\n";
    $dumpfile .= $sqldump;
    $dumpfile .= "\n#the end of backup";

    $filename = 'dcshop_' . Option::DC_VERSION . '_' . date('Ymd_His');
    if ($zipbak == 'y') {
...
if ($action === 'import') {
    ...
    if (getFileSuffix($sqlfile['name']) == 'zip') {
        ...
        if (!file_exists($sqlfile['tmp_name'])) {
            emMsg('只能导入DCSHOP备份的压缩包，且不能修改压缩包文件名！');
        }
    } elseif (getFileSuffix($sqlfile['name']) != 'sql') {
        emMsg('只能导入DCSHOP备份的SQL文件');
    }
...
function checkSqlFileInfo($sqlfile) {
    ...
    fclose($fp);
    if (empty($dumpinfo)) {
        emMsg('该文件不是DCSHOP的数据备份文件');
    }

    if (preg_match("/pro\s\d+\.\d+\.\d+/", $dumpinfo[0], $matches)) {
        $v = $matches[0];
        if ($v !== Option::DC_VERSION) {
            emMsg('不是生成的数据备份，请安装 DCSHOP ' . $v . ' 导入。');
        }
    } else {
        emMsg('该文件不是 DCSHOP pro 的数据备份文件');
    }
